...
| Gap | Impact | Effort |
| --- | --- | --- |
| **No MLflow integration in code.** An extensive interoperability doc exists (`docs/guides/mlflow-interoperability.md`, 575+ lines) but zero lines of MLflow code in the server. No `mlflow` in dependencies. No `MLflowTracingMiddleware`. No `mlflow_enabled` setting. MLflow has a native Deep Agent integration that would work with one line of code. | High | ~20 lines (Phase 1) to ~500 lines (all phases) |
