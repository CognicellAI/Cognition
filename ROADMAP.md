...
| 2B | `tool-safety`, `context-controls`, `async-subagents`, `model-profiles` | `0.10.0-w2b` | Partial complete — `tool-safety` merged and validated on dev K8s (`46 passed`, `2 skipped`), then stabilized by PR #123 against local Docker Compose (`58 passed`, `1 skipped` focused e2e); `context-controls` merged via PR #124; experimental `async-subagents` MVP completed on `codex/async-subagents` for merge; `model-profiles` deferred to last |
