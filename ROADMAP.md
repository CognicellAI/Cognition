...
| Task | Layer | Status | Acceptance Criteria | Effort | Dependencies |
|------|-------|--------|---------------------|--------|--------------|
| **Deep Agents-native default tool surface** | Layer 4 | Completed | Cognition no longer defines, exports, documents, tests, or implicitly injects `webfetch`, `websearch`, or `inspect_package`; an agent with no explicitly attached tools receives only the tools supplied by Deep Agents defaults and middleware; API-registered and MCP tools remain opt-in builder configuration; migration notes identify removed imports and tool names. | 0.5 day | Deep Agents tool contract |
