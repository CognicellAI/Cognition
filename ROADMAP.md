...
| 2026-06-15 | Fix per-agent `blocked_tools` API persistence and scoped PATCH reload — Kennel Wá Salon dev probe saw `PATCH /agents/{name}` return 200 while follow-up GET/raw `config_entities` did not expose `blocked_tools`; regression coverage now verifies POST/PATCH/GET/raw ConfigRegistry round-trip, including scoped agents, plus runtime forwarding into `CognitionAgentParams`. Verification: `uv run pytest tests/unit/api/test_agents_crud.py -q`, targeted field-wiring pytest, `uv run ruff check` on touched files, and `git diff --check` passed locally. | Wá Salon Builder Starter dev report | 2/4/6 | Completed |
...
| Material for MkDocs documentation site | Layer 6 | Completed | Existing Markdown docs build as a navigable Material for MkDocs site with strict build validation, homepage, concepts/guides/blueprints navigation, and configuration ready for `https://cognition.cognicellai.com/docs/` | 0.5 day | None |
| Agent development curriculum scaffold | Layer 6 | Completed | MkDocs exposes a top-level Learn section with Foundations, Core, Production, and Labs tracks; teaches agent-development patterns with Cognition as the exercise backend; includes a reusable lesson template, Support Agent capstone scaffold, and strict docs build validation | 0.5 day | Material for MkDocs documentation site |
| Standalone Cognition web surfaces | Layer 6 | Completed | `cognition.cognicellai.com` serves a full landing page at `/`, a standalone Learn site at `/learn/`, and product docs at `/docs/`; shared build automation validates all static outputs and preserves strict MkDocs builds | 1 day | Material for MkDocs documentation site; Agent development curriculum scaffold |
