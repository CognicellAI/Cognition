...
| **v0.10.0: A2A server adapter** | Layer 4/6/7 | Completed | Cognition exposes itself as an A2A Server via `a2a-sdk`. JSON-RPC `SendMessage` and `SendStreamingMessage` map to Cognition sessions/runs. Agent Card auto-generated from configured agents at `/.well-known/agent-card.json`. Scope enforcement via `A2A-Version: 1.0` + existing `X-Cognition-Scope-*` headers. A2A `contextId` maps to `session_id`; `taskId` maps to `run_id`. HITL pauses surface as `INPUT_REQUIRED`. Push notifications, gRPC, outbound A2A client, and file/media parts deferred. | 2–3 days | P1: runner-lifecycle |
