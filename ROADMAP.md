...
| **Expose Deep Agents context controls (`tool_token_limit_before_evict`, summarization tool middleware)** | Layer 4 | In Progress | Agent config can set `tool_token_limit_before_evict`; declarative middleware supports `summarization_tool` via Deep Agents factory | 0.5 day | deepagents >= 0.4.8 |
