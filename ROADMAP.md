...
|------|-------------|-------|----------------|--------|
| 2026-08-03 | **Durable object publication and selected-backend readiness** — persist opaque object keys, SHA-256 digests, and byte sizes in database artifact manifests; publish immutable digest-addressed S3-compatible bodies before atomically activating manifests; verify integrity after upload and on read; and make readiness fail closed when the builder-selected database or object store is unavailable. | 2/6/7 | Alembic adds nullable durable-object metadata to artifact rows. Database-only local/development bodies remain supported. S3-configured writes use the new manifest contract immediately; Cognition never falls back to a different backend. | In Review |
| 2026-08-03 | **Agent-owned Skill bundles** — make complete Deep Agents-compatible Skill bundles part of each immutable Agent definition and revision; pin Skill content with the run manifest; remove the standalone Skill registry, file-source seeding, and `/skills` CRUD surface. | 2/4/6 | Breaking change: builders redeploy each Skill inside its target Agent definition. Existing standalone Skill records and workspace Skill sources are not read, converted, or exposed through a compatibility API. | Implemented on `release/v0.14.0`; release validation pending |
