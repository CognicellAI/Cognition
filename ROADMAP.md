...
| **v0.10.0: Context compression + offloading + token controls** | Layer 2/4/6/7 | In progress | Wave 2B branch `codex/context-controls` started after PR #123 stabilization. First slice adds declarative `ContextPolicy`, Deep Agents `SummarizationToolMiddleware` alignment, Deep Agents summarization-state translation into `context` SSE events, Prometheus/OTel context signals, and scoped redacted context debug metadata. Remaining work: deterministic verification for large-tool-output offload and richer summarization/offload artifact IDs as upstream/runtime signals become available. | 2–3 days | P1: artifacts-handoffs, P1: runner-lifecycle |
...
| 2B | `tool-safety`, `context-controls`, `async-subagents`, `model-profiles` | `0.10.0-w2b` | Partial complete — `tool-safety` merged and validated on dev K8s (`46 passed`, `2 skipped`), then stabilized by PR #123 against local Docker Compose (`58 passed`, `1 skipped` focused e2e); `context-controls` is in progress on `codex/context-controls`; `async-subagents` remains planned; `model-profiles` deferred to last |
