...
| Task | Layer | Status | Acceptance Criteria | Effort | Dependencies |
|------|-------|--------|---------------------|--------|--------------|
| **v0.11.0: Scoped configuration inheritance** | Layer 1/2/4/6/7 | Planned | Builders can keep arbitrary `effective_scope: dict[str, str]` keys and optionally declare a deterministic `scope_hierarchy` for config inheritance; ConfigRegistry resolution supports global → parent → child override semantics for agents, tools, skills, model/provider profiles, MCP servers, middleware defaults, and sandbox/runtime profiles where applicable; runtime data remains exact-scope owned for sessions, runs, messages, events, approvals, artifacts, memory, sandboxes, callbacks, logs, metrics, and traces; partial parent scopes do not imply admin access to child runtime data; observability reports config source attribution with raw scope values redacted from public events; tests cover global/parent/child/sibling behavior and exact runtime isolation. Strategy: `docs/blueprints/scoped-configuration-inheritance.md`. | 3–5 days | Canonical `effective_scope` propagation; ConfigRegistry; runtime durability foundation |
