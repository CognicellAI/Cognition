...
|------|-------------|-------|----------------|--------|
| 2026-08-03 | **v0.14.0 Agent-owned MCP configuration boundary** — move MCP server definitions from global ConfigRegistry/server CRUD into immutable Agent definitions; load tools per server from the pinned runtime Agent snapshot; remove model-visible scope injection; make optional server discovery failures non-fatal while required failures surface typed, redacted errors; reject duplicate canonical tool identities; and expose auth as opaque `none`, `oauth`, or `outbound_provider` profile configuration rather than persisted credentials. | 2/4/6/7 | Breaking change: remove `/mcp-servers` as a runtime management surface. Builders must compile approved MCP routes into each Agent's `mcp.servers` block and use Agent revisions/digests to promote config. Credential resolution remains outside Cognition core; direct OAuth and custom outbound providers are represented as profiles and must not expose credentials in Agent config, model input, logs, traces, telemetry, or readiness projections. | In Review |
