...
@pytest.mark.asyncio
async def test_lists_only_agent_owned_skill_directories(backend: AgentSkillsBackend) -> None:
    result = await backend.als_info("/")
    assert result == [
        {"path": "/review/", "is_dir": True, "size": 0, "modified_at": ""}
    ]
...
async def test_downloads_primary_and_supporting_files(backend: AgentSkillsBackend) -> None:
    result = await backend.adownload_files(
        ["/review/SKILL.md", "/review/references/guide.md"]
    )
...
async def test_missing_and_traversal_paths_are_redacted(backend: AgentSkillsBackend) -> None:
    result = await backend.adownload_files(
        ["/missing/SKILL.md", "/review/../secret.txt"]
    )
...
@pytest.mark.asyncio
async def test_reads_with_line_numbers_and_bounds(backend: AgentSkillsBackend) -> None:
    result = await backend.aread("/review/SKILL.md", offset=1, limit=2)
    assert result.error is None
    assert result.file_data is not None
    assert result.file_data["content"].splitlines()[0].startswith("     2\t")
