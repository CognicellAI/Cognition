...
`mcp_oauth` is the normal direct-provider choice: it does not require a
builder-installed provider. `outbound_auth_provider` is an opt-in alternative
only where a builder needs an egress gateway, live policy decision, credential
broker, or workload-token exchange.
...
For a first authorization, the trusted builder calls
`POST /agents/{agent}/mcp/{alias}/oauth/authorize` with the same authorized
scope it will use at runtime. Cognition verifies that the exact resolved Agent
binding is `mcp_oauth`, begins the SDK's PKCE flow, and returns the provider
authorization URL. The configured public callback URL must route to Cognition's
`GET /agents/mcp/oauth/callback`. The callback accepts only the provider's
one-time `code` and opaque `state`: it never accepts Agent identity, endpoint,
or scope from the browser. State is HMAC-addressed, short-lived, one-time, and
the callback code is encrypted in the configured database so a separate API
replica can deliver it to the worker that initiated the SDK flow.
