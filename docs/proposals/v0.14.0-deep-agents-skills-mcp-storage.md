...
| Complete Agent-owned Skill bundles | Database, inside the immutable Agent revision |
...
CompositeBackend(
    ...
    routes={
        ...
        "/skills/": pinned_agent_revision_backend,
...
|---|---|---|
| [ADR-0003](../architecture/decisions/0003-agent-owned-capability-revisions.md) | Agent-owned capability revisions | Atomic skills/MCP deployment, run consistency, audit, and cache identity without a public historic-version selector |
...
| Single-string registered skill content | Complete `SKILL.md` bundle with supporting files inside the Agent revision |
...
global MCP subsystem, Agent-owned Skill bundles, and removal of the standalone
Skill registry are implemented. Digest-addressed S3 publication, complete
...
| Wave 3 | Add manifests, S3 backend, durable routes, storage health, and no-host-persistence deployment tests |
