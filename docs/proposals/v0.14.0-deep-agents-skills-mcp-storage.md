...
per-server discovery, readiness, OAuth persistence and handoff, and removal of
the global MCP subsystem are implemented. Agent-owned Skill bundles and removal
of the standalone Skill registry are in validation. The remaining implementation
work is the durable-storage acceptance pass and release validation.
