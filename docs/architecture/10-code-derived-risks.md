...
| AR-015 | Open | Empty Agent tool allow-list loads every enabled visible registry tool | “No selected tools” does not mean no registry tools | `agent/resolver.py::RuntimeResolver.build_tools` |
| AR-016 | Accepted constraint — unsafe opt-in | Registry Python tool source is compiled/executed in the server process | API Python tools are rejected in strict production mode and require explicit unsafe development configuration | `tests/unit/test_v013_strict_execution.py`; ADR-0001 |
